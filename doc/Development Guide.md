...
## 2. 核心機能：人間模倣アルゴリズム (Human-Like Behavior)

Bot認識を回避するための必須要件です。

### A. 揺らぎのあるスクロール (Variable Scrolling)
一定のパターンを排除します。
- **実装:** `window.scrollBy` に加え、`delta` と `delay` を `Math.random()` で動的に算出。
- **マイクロインタラクション:** スクロール中に一度「少し戻る」ような動作を混ぜることで、より人間らしさを演出します。

### B. 視認判定に基づいた取得 (Visibility-Based Extraction)
- **実装:** `IntersectionObserver` を使用。
- **目的:** レンダリングされていないDOMへのアクセスを最小限に抑え、ブラウザのパフォーマンスへの影響を減らすとともに、X側の解析ロジックとの矛盾を防ぎます。
